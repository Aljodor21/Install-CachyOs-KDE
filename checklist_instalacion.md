...
Al final del install, se lanza `lib/post_install_config.sh`. Son 6 pasos:
...
> Claude Code CLI y opencode **no están en el wizard** — se instalan pero su
> auth es 100% manual: corré `claude` o `opencode` cuando quieras.
...
Mismo wizard de 6 pasos (ver Camino A, sección 3).
...
> Claude Code CLI y opencode NO están en el wizard. Se instalan con el
> script pero su auth es 100% manual: corré `claude` o `opencode` cuando
> quieras y seguí las instrucciones.
